    ...
    <div className="mt-4 pb-3 3xl:mt-6">
      {menuItems.map((item, index) => {
        ...
        const pathnameExistInDropdowns: any = (
          item?.dropdownItems as {
            name: string;
            href: string;
            badge?: string;
          }[]
        )?.filter((dropdownItem) => dropdownItem.href === pathname);
        // ?.filter((dropdownItem) => dropdownItem.href === "/");
        const isDropdownOpen = Boolean(pathnameExistInDropdowns?.length);
        const isAdminMenu = item?.menuType === "admin";
        // const isHideMenu = role !== "ROLE_ADMIN" && isAdminMenu;
        const isHideMenu = isAdminMenu;
              ...
              <>
                {item?.dropdownItems ? (
                  ...
                  >
                    ...
                    {(item?.badge as string)?.length ? (
                      <StatusBadge status={item?.badge as string} />
                    ) : null}
                  </Link>
                )}
              </>
